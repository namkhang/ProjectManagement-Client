...
const ChatPageForAdmin = (props) => {
  ...
  useEffect(()=>{
      async function getData(){
            let response = await axios.get(`http://localhost:5000/user/list-chat/${Cookies.get("adminID")}` , {headers : {
              "Authorization" : `Bearer ${localStorage.getItem("token")}`
            }})
            let receiverInformation = []
            response.data.dataChatForUser.forEach((i) => {
              let result = i.chat.filter(x => x.userID_chat !== Cookies.get("adminID"))
  ...
  function enterChat(event){
    if(event.keyCode === 13){
      if(chats.length !== 0){
        socket.emit("chat" , {
          userID_chat : Cookies.get("adminID"),
                        ...
                        <>
                          <div className="chat_people" onClick={() => getChat(item._id)} key={index}>
                            ...
                            <div className="chat_ib">
                              ...
                              <p>{item.chat[item.chat.length - 1].userID_chat === Cookies.get("adminID") ? item.chat[item.chat.length - 1].chat_content !== "" ?   `Bạn: ${item.chat[item.chat.length - 1].chat_content}` : "" :  item.chat[item.chat.length - 1].chat_content}</p>
                            </div>
                          </div>
                        </>
                      );
              ...
              <div className="mesgs">
                <div className="msg_history">
                  <div>
                    ...
                    ""
                    :
                      chat.userID_chat === Cookies.get("adminID") ? 
...
export default ChatPageForAdmin;

...
const MyReportTempalte = (props) => {
    const [reportTemplate , setReportTemplate] = useState([])
    const [loading , setLoading] = useState(false)

    useEffect(()=>{
      async  function getData(){
        let userID = Cookies.get("mentorID")
        let response = await axios.get(`http://localhost:5000/admin/my-reporttemplate/${userID}` , {headers : {
    ...
    function ToDetailReportTemplate(reportTemplateID){
            window.location.href = `http://localhost:3000/mentor/edit-reporttemplate/${reportTemplateID}`
    }
...
export default MyReportTempalte;
